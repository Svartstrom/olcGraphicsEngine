...
struct vec3d
{
    ...
    vec3d(int _x)   : x(_x), y(_x), z(_x), w(1.0) {}
    vec3d(float _x) : x(_x), y(_x), z(_x), w(1.0) {}
    vec3d(float _x, float _y, float _z) : x(_x), y(_y), z(_z), w(1.0) {}
    vec3d(int _x, int _y, int _z) : x((float)_x), y((float)_y), z((float)_z), w(1.0) {}

    /*vec3d operator * (static mat4x4 &rhs)
    {
        vec3d out;
        // o.x = i.x * m.m[0][0] + i.y * m.m[1][0] + i.z * m.m[2][0] + m.m[3][0];
        out.x = this->x * rhs->m[0][0] + this->y * rhs->m[1][0] + this->z * rhs->m[2][0];
        out.y = ;
        out.z = ;
